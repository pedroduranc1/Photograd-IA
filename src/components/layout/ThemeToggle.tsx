import { Pressable, View } from 'react-native';
import { setAndroidNavigationBar } from '~/src/utils/android-navigation-bar';
import { MoonStar } from '~/src/components/ui/icons/MoonStar';
import { Sun } from '~/src/components/ui/icons/Sun';
import { useColorScheme } from '~/src/hooks/ui/useColorScheme';

export function ThemeToggle() {
  const { isDarkColorScheme, setColorScheme } = useColorScheme();

  function toggleColorScheme() {
    const newTheme = isDarkColorScheme ? 'light' : 'dark';
    setColorScheme(newTheme);
    setAndroidNavigationBar(newTheme);
  }

  return (
    <Pressable
      onPress={toggleColorScheme}
      className='web:ring-offset-background web:transition-colors web:focus-visible:outline-none web:focus-visible:ring-2 web:focus-visible:ring-ring web:focus-visible:ring-offset-2 active:opacity-70'
    >
      <View className='flex-1  aspect-square justify-center items-center web:px-5'>
        {isDarkColorScheme ? (
          <MoonStar className='text-foreground' size={23} strokeWidth={1.25} />
        ) : (
          <Sun className='text-foreground' size={24} strokeWidth={1.25} />
        )}
      </View>
    </Pressable>
  );
}
